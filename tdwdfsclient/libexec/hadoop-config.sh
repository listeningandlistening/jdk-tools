...
RAID_DIR=${RAID_DIR:-"share/hadoop/raid"}
RAID_LIB_JARS_DIR=${RAID_LIB_JARS_DIR:-"share/hadoop/raid/lib"}
...
# put raid in classpath if present
if [ "$HADOOP_RAID_HOME" = "" ]; then
  if [ -d "${HADOOP_PREFIX}/$RAID_DIR" ]; then
    export HADOOP_RAID_HOME=$HADOOP_PREFIX
  fi
fi

if [ -d "$HADOOP_RAID_HOME/$RAID_DIR/webapps" ]; then
  CLASSPATH=${CLASSPATH}:$HADOOP_RAID_HOME/$RAID_DIR
fi

if [ -d "$HADOOP_RAID_HOME/$RAID_LIB_JARS_DIR" ]; then
  CLASSPATH=${CLASSPATH}:$HADOOP_RAID_HOME/$RAID_LIB_JARS_DIR'/*'
fi
